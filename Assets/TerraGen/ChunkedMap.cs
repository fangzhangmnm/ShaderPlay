...
namespace fzmnm.InfiniteGeneration
{
    ...
    public class ChunkedMap<T> : ChunkedMapBase where T:struct
    {
    ...
    public class AreaMap<T>
    {
        ...
        public void Reset(Vector2Int newMin, Vector2Int newMax)
        {
            ...
            if (newSize != size || buffer is null)
    ...
    public class ChunkedEntityMap<T> : ChunkedMapBase
    {
        ...
        Dictionary<Vector2Int, List<Entity>> chunks = new Dictionary<Vector2Int, List<Entity>>();
        ...
        //}
        public void Add(Vector2Int coord, T payload)
        {
            Vector2Int coordc = new Vector2Int(DivFloor(coord.x, chunkSize), DivFloor(coord.y, chunkSize));
            if (!chunks.TryGetValue(coordc, out List<Entity> chunk))
                chunk = chunks[coordc] = new List<Entity>();
            chunk.Add(new Entity { coord=coord,payload=payload });
        }
        public void Remove(Entity entity)
        {
            Vector2Int coordc = new Vector2Int(DivFloor(entity.coord.x, chunkSize), DivFloor(entity.coord.y, chunkSize));
            if (!chunks.TryGetValue(coordc, out List<Entity> chunk))
                return;
            chunk.Remove(entity);
        }
        public IEnumerable<Entity> Read(Vector2Int min, Vector2Int max)
        {
            ...
            for (coordc.y = minc.y; coordc.y <= maxc.y; ++coordc.y)
                for (coordc.x = minc.x; coordc.x <= maxc.x; ++coordc.x)
                {
                    if (!chunks.TryGetValue(coordc, out List<Entity> chunk))
                        continue;
                    foreach (Entity entity in chunk)
                        if(entity.coord.x>=min.x && entity.coord.x<=max.x && entity.coord.y>=min.y && entity.coord.y<=max.y)
                            yield return entity;
                }
        }
    }
