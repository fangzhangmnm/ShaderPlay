...
namespace fzmnm.Test
{
    public class TestGenerationManager : MonoBehaviour
    {
        ...
        public void GenerateMesh()
        {
            ...
            var map = new AreaMap<float>(min, max);
            //Debug.Log(map.buffer.Length);
            //Debug.Log(resolution);
            mgr.ReadMap("height", map);
            ...
            mesh.Clear();
            mesh.vertices = GenerateVerticesArray(resolution, new Vector3(min.x,0,min.y) * unitLength, unitLength, map.buffer,map.yStride, stride);
