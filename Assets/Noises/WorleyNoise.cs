...
namespace fzmnm
{
    public static partial class Noise
    {
        ...
        public static float WorleyOctave3D(float x, float y, float z, int octave, float persistence = .5f, float lacunarity = 2, int repeatX = 256, int repeatY = 256, int repeatZ = 256, byte seed = 0)
        {
            float value = 0;
            float amplitude = 1;
            for (int i = 0; i < octave; ++i)
            {
                value += (Worley3D(x, y, z, repeatX, repeatY, repeatZ, (byte)(seed + i)) - .5f) * 2 * amplitude;
        ...
        public static float WorleyOctave2D(float x, float y, int octave, float persistence = .5f, float lacunarity = 2, int repeatX = 256, int repeatY = 256, byte seed = 0)
        {
            seed &= 0xff;
            float value = 0;
            float amplitude = 1;
            for (int i = 0; i < octave; ++i)
            {
                value += (Worley2D(x, y, repeatX: repeatX, repeatY: repeatY, (byte)(seed + i)) - .5f) * 2 * amplitude;
                amplitude *= persistence;
                x *= lacunarity;
                y *= lacunarity;
                repeatX = Mathf.RoundToInt(repeatX * lacunarity);
        ...
        public static float Worley2D(float x, float y, int repeatX = 256, int repeatY = 256, byte seed = 0)
        {
        ...
        public static float Worley3D(float x, float y, float z, int repeatX = 256, int repeatY = 256, int repeatZ = 256, byte seed = 0)
        {
            int xi = Mathf.FloorToInt(x); float xf = x - xi; xi = PositiveMod(xi, repeatX) & 0xff;
            int yi = Mathf.FloorToInt(y); float yf = y - yi; yi = PositiveMod(yi, repeatY) & 0xff;
