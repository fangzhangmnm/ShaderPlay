...
public static class Perlin
{
    ...
    public static float Noise(float x)
    {
        ...
        x -= Mathf.Floor(x);
        var u = Fade(x);
        return Lerp(u, Grad(perm[X], x), Grad(perm[(X + 1) & 0xff], x - 1)) * 2;
    }
    ...
    public static float Noise(float x, float y, float z)
    {
        ...
        var AB = (perm[(A + 1) & 0xff] + Z) & 0xff;
        var BB = (perm[(B + 1) & 0xff] + Z) & 0xff;
        return Lerp(w, Lerp(v, Lerp(u, Grad(perm[AA], x, y, z), Grad(perm[BA], x - 1, y, z)),
                               Lerp(u, Grad(perm[AB], x, y - 1, z), Grad(perm[BB], x - 1, y - 1, z))),
                       Lerp(v, Lerp(u, Grad(perm[(AA + 1) & 0xff], x, y, z - 1), Grad(perm[(BA + 1) & 0xff], x - 1, y, z - 1)),
                               Lerp(u, Grad(perm[(AB + 1) & 0xff], x, y - 1, z - 1), Grad(perm[(BB + 1) & 0xff], x - 1, y - 1, z - 1))));
    }
